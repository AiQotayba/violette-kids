...
export const categoriesController = {
  async listPublic(req: Request, res: Response): Promise<void> {
    ...
    const result = await categoriesService.getList({ limit: safeLimit, offset: safeOffset, search });
  ...
  async getById(req: Request, res: Response): Promise<void> {
    ...
    const item = await categoriesService.getById(parsed.data.id);
    if (!item) {
  ...
  async create(req: Request, res: Response): Promise<void> {
    ...
    try {
      const created = await categoriesService.create(parsed.data);
      sendSuccess(res, created, 201);
    } catch (e) {
      const err = e as Error & { code?: string };
      if (err.code === "P2002") sendError(res, "Category with this name already exists", 409);
  ...
  async update(req: Request, res: Response): Promise<void> {
    ...
    try {
      const updated = await categoriesService.update(paramParsed.data.id, bodyParsed.data);
      sendSuccess(res, updated);
    } catch (e) {
      const err = e as Error & { code?: string };
      if (err.code === "P2025") sendError(res, "Category not found", 404);
      else if (err.code === "P2002") sendError(res, "Category with this name already exists", 409);
      else sendError(res, (e as Error).message, 500);
    }
  },
  ...
  async delete(req: Request, res: Response): Promise<void> {
    ...
    try {
      await categoriesService.delete(parsed.data.id);
      res.status(204).send();
    } catch (e) {
      const err = e as Error & { code?: string };
      if (err.code === "P2025") sendError(res, "Category not found", 404);
